...
import { Eye, EyeOff, ArrowRight, Mail, Lock, User, Check, Sparkles } from "lucide-react"
...
export default function RegisterPage() {
  ...
  const isFormValid = formData.name && formData.email && passwordStrong && passwordMatch && agreeTerms
    ...
    <div className="animate-appear">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Cria uma nova conta e começa a explorar
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {formData.password && (
            <div className={`text-xs flex items-center gap-1.5 ${passwordStrong ? 'text-emerald-400' : 'text-amber-400'}`}>
              <div className={`w-1 h-1 rounded-full ${passwordStrong ? 'bg-emerald-400' : 'bg-amber-400'}`} />
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              onChange={handleChange}
              className={`pl-10 pr-10 h-11 ${formData.confirmPassword && (passwordMatch ? 'border-emerald-500/50' : 'border-red-500/50')}`}
              required
            />
            <button
              type="button"
          ...
          {formData.confirmPassword && (
            <div className={`text-xs flex items-center gap-1.5 ${passwordMatch ? 'text-emerald-400' : 'text-red-400'}`}>
              {passwordMatch ? (
                <>
                  <Check className="w-3.5 h-3.5" />
                  Passwords correspondem
                </>
              ) : (
                <>
        ...
        <div className="flex items-start gap-3 py-2">
          <input
            ...
            className="mt-1.5 w-4 h-4 rounded border border-border accent-primary cursor-pointer"
          />
